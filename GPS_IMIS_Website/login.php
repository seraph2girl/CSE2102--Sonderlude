...
$name_err = $username_err = $password_err = $position_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($username_err) && empty($password_err) && empty($position_err)){
        ...
        $sql = "SELECT id, name, username, password, position FROM users WHERE username = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $name, $username, $hashed_password, $position);
...
<body>
    <div class="wrapper">
        <h2><center>Login to IMIS</center></h2>
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
               ...
               <center> <input type="password" name="password" class="form-control"></center>
            ...
            <p>Don't have an account? <a href="register.php">Register to gain access</a>.</p>
        </form>
    </div>
</body>
</html>
